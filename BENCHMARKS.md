...
![FFT Throughput](./charts/fft_throughput.png)

#### Results Summary

| Library | Input Size | Throughput | Backend |
|---------|------------|------------|---------|
| dspx | small | 132.13M samples/sec | CPU (Native C++ SIMD) |
| tfjs | small | 4.17M samples/sec | CPU (TensorFlow.js) |
| fft.js | small | 4.11M samples/sec | CPU (Pure JS) |
| dspx | medium | 219.57M samples/sec | CPU (Native C++ SIMD) |
| tfjs | medium | 14.14M samples/sec | CPU (TensorFlow.js) |
| fft.js | medium | 84.50M samples/sec | CPU (Pure JS) |
| dspx | large | 117.77M samples/sec | CPU (Native C++ SIMD) |
| tfjs | large | 16.08M samples/sec | CPU (TensorFlow.js) |
| fft.js | large | 53.85M samples/sec | CPU (Pure JS) |
...
![FIR Filter Throughput](./charts/fir_throughput.png)

#### Results Summary

| Library | Input Size | Throughput | Backend |
|---------|------------|------------|---------|
| dspx | small | 11.61M samples/sec | CPU (Native C++ SIMD) |
| fili | small | 8.93M samples/sec | CPU (Pure JS) |
| naive_js | small | 13.25M samples/sec | CPU (Pure JS) |
| dspx | medium | 35.57M samples/sec | CPU (Native C++ SIMD) |
| fili | medium | 12.21M samples/sec | CPU (Pure JS) |
| naive_js | medium | 15.69M samples/sec | CPU (Pure JS) |
| dspx | large | 37.81M samples/sec | CPU (Native C++ SIMD) |
| fili | large | 12.16M samples/sec | CPU (Pure JS) |
| naive_js | large | 21.27M samples/sec | CPU (Pure JS) |
...
![Moving Average (Small)](./charts/moving_avg_small.png)

![Moving Average (Medium)](./charts/moving_avg_medium.png)
...
|-------------|-----------|---------------|--------|
| 32 | 2.901 | 7.875 | 2.7x |
| 128 | 2.554 | 30.606 | 12.0x |
| 512 | 2.534 | 122.275 | 48.3x |
| 2048 | 2.599 | 42.910 | 16.5x |
| 8192 | 2.559 | 162.779 | 63.6x |
...
![Redis Latency](./charts/redis_latency.png)

#### Results Summary

| Input Size | Save Time (ms) | Load Time (ms) | State Size | Seamless? |
|------------|----------------|----------------|------------|-----------|
| small | 0.055 | 0.210 | 3.39 KB | ✅ |
| medium | 0.058 | 0.231 | 3.45 KB | ✅ |
| large | 0.046 | 0.202 | 3.38 KB | ✅ |

**Performance Metrics:**
- Average save time: **0.053 ms**
- Average load time: **0.214 ms**
...
![Logging Performance](./charts/logging_perf.png)

#### Overhead Analysis

| Mode | Average Overhead | Recommendation |
|------|------------------|----------------|
| batched | -1.34% | ✅ Recommended |
| per-message | -6.41% | ✅ Recommended |
| console | -13.15% | ✅ Recommended |

#### Detailed Results

| Input Size | Mode | Throughput | Overhead |
|------------|------|------------|----------|
| medium | none | 103.65M samples/sec | — |
| medium | batched | 105.55M samples/sec | -1.80% |
| medium | per-message | 116.25M samples/sec | -10.84% |
| medium | console | 119.34M samples/sec | -13.15% |
| large | none | 141.83M samples/sec | — |
| large | batched | 143.09M samples/sec | -0.88% |
| large | per-message | 144.68M samples/sec | -1.97% |
